...
loglambda=logspace(-4,4,N_points);
...
for lambda_i=1:N_points
    [M_final_hard{lambda_i}, O_final_hard{lambda_i}, U_final_hard{lambda_i}]=Algorithm_1(M_init,O_init,X,U_init_star,N,C,linelambda(lambda_i),q_hard,threshold,print_flag);
    n_outliers_hard(1,lambda_i)= sum(O_final_hard{lambda_i}(1,:)~=0 & O_final_hard{lambda_i}(2,:)~=0,2);
end
display('Hard key-means done')
for lambda_i=1:N_points
    [M_final_soft{lambda_i}, O_final_soft{lambda_i}, U_final_soft{lambda_i}]=Algorithm_1(M_init,O_init,X,U_init_star,N,C,linelambda(lambda_i),q_soft,threshold,print_flag);
    n_outliers_soft(1,lambda_i)= sum(O_final_soft{lambda_i}(1,:)~=0 & O_final_soft{lambda_i}(2,:)~=0,2);
end
display('Soft key-means done')
figure(11)
